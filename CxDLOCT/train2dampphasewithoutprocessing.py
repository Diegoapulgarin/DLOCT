...
def logScale(slices):
    ...
    logslices+= 0.0001
    logslices= np.transpose(logslices, (2, 0, 1, 3))
    nSlices = len(logslices)
...
def summarize_performance(step, g_model, dataset, n_samples=3):
    ...
    for i in range(n_samples):
        invslice0 = inverseLogScale(X_realA[i], smax[i], smin[i])
    ...
    for i in range(n_samples):
        invslice1 = inverseLogScale(X_fakeB[i], smax[i], smin[i])
    ...
    for i in range(n_samples):
        invslice2 = inverseLogScale(X_realB[i], smax[i], smin[i])
...
for t in tqdm(range(len(tomInput))):
    tomNorm,tmax,tmin = logScale(tomTarget[t])
    tomccNorm,imax,imin = logScale(tomInput[t])
...
combined_target = np.concatenate(tomTargetNorm,axis=0)
combined_input = np.concatenate(tomInputNorm,axis=0)
combined_max = np.concatenate(tomTargetmax, axis=0)
combined_min = np.concatenate(tomTargetmin, axis=0)
