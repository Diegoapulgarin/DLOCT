...
#%%
n = 0
test = denormalize_tomogram(tomTargetNorm[n],tomTargetmax[n],tomTargetmin[n])
#%%
...
for i in range(n_samples):
    ...
    plot0 = 10*np.log10(abs((invslice0[:,:,0]+1j*invslice0[:,:,1])+0.0001)**2)
    pyplot.subplot(3, n_samples, 1 + i)
    pyplot.axis('off')
    pyplot.imshow(plot0, cmap='hot', vmin=vmin, vmax=vmax)
...
for i in range(n_samples):
    ...
    plot2 = 10*np.log10(abs((invslice2[:,:,0]+1j*invslice2[:,:,1])+0.0001)**2)
    pyplot.subplot(3, n_samples, 1 + n_samples*2 + i)
    pyplot.axis('off')
    pyplot.imshow(plot2, cmap='hot', vmin=vmin, vmax=vmax)    
    print(smin[i],',',smax[i])
