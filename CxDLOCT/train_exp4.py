...
from tensorflow.keras.layers import Conv1D, Dense, Dropout, BatchNormalization, Input
...
model = Sequential([
    ...
    BatchNormalization(),
    Dropout(0.5),
    Conv1D(filters=2, kernel_size=3, activation='linear', padding='same')  # Ensure output has 2 channels
])
...
# Inicializar un array para almacenar las predicciones reorganizadas
predictions_reorganized = np.zeros((2304, X_test.shape[0], 2))

# Llenar el array con las predicciones
for i in range(X_test.shape[0]):
    predictions_reorganized[:, i, :] = predictions[i]

# Verificar las formas de las predicciones reorganizadas
print("Predictions reorganized shape:", predictions_reorganized.shape)
...
axs[1].plot(abs(fft(X_test_partial[:,0,0])))
axs[1].set_title('Input')
axs[2].plot(abs(fft(y_test_partial[:,0,0]+1j*y_test_partial[:,0,1])))
axs[2].set_title('target')

fig2, axs2 = plt.subplots(nrows=1,ncols=2)
axs2[0].imshow(dbscale(fft(predictions_reorganized[:,:,0]+1j*predictions_reorganized[:,:,1],axis=0)))
axs2[0].set_title('predicted')
axs2[1].imshow(dbscale(fft(y_test_partial[:,:,0]+1j*y_test_partial[:,:,1],axis=0)))
axs2[1].set_title('target')
...
# model.save('model_1d_cnn_simplified.h5')
