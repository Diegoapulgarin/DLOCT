...
def summarize_performance(step, g_model, dataset, n_samples=3):
    ...
    for i in range(n_samples):
        array = X_realA[i]
        invslice0 = fast_reconstruct(array[:,:,0]*np.exp(1j*array[:,:,1]))
    ...
    for i in range(n_samples):
        array = X_fakeB[i]
        invslice1 = fast_reconstruct(array[:,:,0]*np.exp(1j*array[:,:,1]))        
    ...
    for i in range(n_samples):
        array = X_realB[i]
        invslice2 = fast_reconstruct(array[:,:,0]*np.exp(1j*array[:,:,1]))
...
fringes_target = fftshift(ifft(fftshift(all_targets_partioned,axes=0),axis=0),axes=0)
# Normalize
n_bscans = fringes_target.shape[2]
mean_vals_target = np.zeros(n_bscans,dtype=np.complex_)
std_vals_target = np.zeros(n_bscans,dtype=np.complex_)
fringes_target_normalized = np.zeros_like(fringes_target,dtype=np.complex_)
for i in range(n_bscans):
    mean_vals_target[i] = np.mean(fringes_target[:, :, i])
    std_vals_target[i] = np.std(fringes_target[:, :, i])
    fringes_target_normalized[:, :, i] = (fringes_target[:, :, i] - mean_vals_target[i]) / std_vals_target[i]
del all_targets_partioned
fringes_tomograms = abs(fftshift(ifft(fftshift(all_tomograms_partioned,axes=0),axis=0),axes=0))
# Normalize
n_bscans = fringes_tomograms.shape[2]
mean_vals = np.zeros(n_bscans)
std_vals = np.zeros(n_bscans)
fringes_tomograms_normalized = np.zeros_like(fringes_tomograms)
for i in range(n_bscans):
    mean_vals[i] = np.mean(fringes_tomograms[:, :, i])
    std_vals[i] = np.std(fringes_tomograms[:, :, i])  
    fringes_tomograms_normalized[:, :, i] = (fringes_tomograms[:, :, i] - mean_vals[i]) / std_vals[i]
del all_tomograms_partioned
tomograms_hilbert = hilbert(fringes_tomograms_normalized,axis=0)
as_tomograms = fringes_tomograms_normalized+1j*tomograms_hilbert

amp_target = abs(fringes_target_normalized)
phase_target = np.angle(fringes_target_normalized)
amp_tomograms = abs(as_tomograms)
phase_tomograms = np.angle(as_tomograms)
del fringes_target_normalized,as_tomograms
bscan = 19
X = np.stack((amp_tomograms,phase_tomograms),axis=3)
X = np.transpose(X,(2,0,1,3))
Y = np.stack((amp_target,phase_target),axis=3)
Y = np.transpose(Y,(2,0,1,3))
del amp_target,phase_target,amp_tomograms,phase_tomograms
dataset = [X,Y]
shape_tom = np.shape(X)
image_shape = (shape_tom[1],shape_tom[2],2)
#%%
# define the models
