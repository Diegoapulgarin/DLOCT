...
def calcular_senal_analitica(fringes):
    """
    Calcula la señal analítica para cada franja en el eje z.

    :param fringes: Array de las franjas reales con dimensiones (n, z, x, y).
    :return: Array de la señal analítica con las mismas dimensiones.
    """
    # Inicializar el array para la señal analítica con números complejos
    senal_analitica = np.zeros_like(fringes, dtype=complex)

    # Iterar sobre las dimensiones n, x, y y calcular la transformada de Hilbert para cada z
    for n in range(fringes.shape[0]):
        for x in range(fringes.shape[2]):
            for y in range(fringes.shape[3]):
                # Seleccionar la franja específica
                franja_actual = fringes[n, :, x, y]
                # Calcular la señal analítica (z + 1j * H{s(z)})
                senal_analitica[n, :, x, y] = franja_actual + 1j * hilbert(franja_actual)

    return senal_analitica

# Suponiendo que 'fringes' es tu array de franjas reales
# fringes = ...
#%%
# Calcular la señal analítica
analytic_signal = calcular_senal_analitica(abs(fringes)*np.cos(np.angle(fringes)))
...
for i in range(np.shape(fringes)[0]):
    fftfringes,_ = reconstruct_tomogram(analytic_signal[i,:,:,:],z=2)
...
#%% pad to big tomograms comented cause we need probe first the model and the theory with small tomograms 
...
#%%
padded_tomogram_train =  normalized_volume_real[:-1]
padded_target_train = normalized_volume_complex[:-1]
...
padded_tomogram_test = normalized_volume_real[-1]
padded_target_test = normalized_volume_complex[-1]
