...
fig.update_layout(
    yaxis=dict(title='MSE Value', range=[0, 0.25], showgrid=True, zeroline=True, showline=True, gridcolor='rgba(128,128,128,0.3)', zerolinecolor='rgba(128,128,128,0.5)', linecolor='rgba(0,0,0,1)'), # Añadimos la malla y la línea del eje y
...
fig = make_subplots(rows=1, cols=2, subplot_titles=("SSIM Metric", "MSE Metric"))


fig.add_trace(go.Scatter(y=ssim_metric, mode='lines+markers', name='SSIM', line=dict(width=2.5)), row=1, col=1)
...
fig.add_trace(go.Scatter(y=mse_values, mode='lines+markers', name='MSE', line=dict(width=2.5)), row=1, col=2)
fig.add_trace(go.Scatter(y=mse_values + mse_std_values, fill='tonexty', fillcolor='rgba(0,100,80,0.2)', line=dict(color='rgba(255,255,255,0)'), name='+1 STD', showlegend=False), row=1, col=2)
fig.add_trace(go.Scatter(y=mse_values - mse_std_values, fill='tonexty', fillcolor='rgba(0,100,80,0.2)', line=dict(color='rgba(255,255,255,0)'), name='-1 STD',showlegend=False), row=1, col=2)

for epoch, value in [(11, mse_values[11]), (4, mse_values[4]), (20, mse_values[20])]:
    annotation_text = f"{value:.4f}"
    fig.add_annotation(x=epoch, y=value+0.02*value, text=annotation_text, showarrow=True, arrowhead=4, ax=0, ay=-20, row=1, col=2)
...
fig.update_layout(
    yaxis1=dict(title='SSIM Value', range=[0, 1],showgrid=False, zeroline=True, showline=True, gridcolor='rgba(128,128,128,0.3)', zerolinecolor='rgba(128,128,128,0.5)', linecolor='rgba(0,0,0,1)'),
    yaxis2=dict(title='MSE Value', range=[0, 1],showgrid=False, zeroline=True, showline=True, gridcolor='rgba(128,128,128,0.3)', zerolinecolor='rgba(128,128,128,0.5)', linecolor='rgba(0,0,0,1)'),
    xaxis1=dict(title='Epoch', showgrid=False, zeroline=False, showline=True, gridcolor='rgba(128,128,128,0.3)', zerolinecolor='rgba(128,128,128,0.5)', linecolor='rgba(0,0,0,1)'),
    xaxis2=dict(title='Epoch', showgrid=False, zeroline=False, showline=True, gridcolor='rgba(128,128,128,0.3)', zerolinecolor='rgba(128,128,128,0.5)', linecolor='rgba(0,0,0,1)'),
    font=dict(family="Times New Roman", size=14),
    plot_bgcolor='rgba(0,0,0,0)'
)
